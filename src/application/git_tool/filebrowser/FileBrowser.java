...
public class FileBrowser extends JPanel {
    ...
    private File[] getContent() {
        try{
            String[] contentString = this.gitTool.getProcessBuilder().directory().list();
            ...
            for(String s: contentString) {
                File file = new File(this.gitTool.getProcessBuilder().directory().getAbsoluteFile()+"/"+s);
                if(file.isDirectory()) {
                    content[i++] = file;
                } else {
                    files.add(file);
    ...
    private void openStartPath(JTree tree) {
        if(this.gitTool.getProcessBuilder().directory().getAbsolutePath().equals("/")) {
            return;
        }
        String[] pathParts = this.gitTool.getProcessBuilder().directory().getAbsolutePath().substring(1).split(File.separator);
